...
class Calulates implements Function<List<Double>, Result>{

	public Result apply(List<Double> arr)
	{
		Result r = new Result();
...
class Roots{ 
	...
	public static void main(String args[]){
		...
		if((args.length == 3) && (args[0].matches(reg) && (args[1].matches(reg) && args[2].matches(reg)))){
			...
			array.add(n2);
			array.add(n3);
			Function<List<Double>, Result> fun = new Calulates();
			Result re = fun.apply(array);
			if((2 * array.get(0)) > 0){
